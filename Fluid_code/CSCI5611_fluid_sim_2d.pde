...
int numParticles = plist.length; //Maximum number 
int totalParticles = 0; //counter for generation/current number of balls
...
void draw() {
  ...
  scene_scale = width / 10.0f;
    ...
    for(int i = 0; i < totalParticles; i++){ 
      ...
      ellipse(plist[i].pos.x*scene_scale,plist[i].pos.y*scene_scale, nodeR*scene_scale, nodeR*scene_scale); 
